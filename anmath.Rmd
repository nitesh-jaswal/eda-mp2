...
With the exception of immigration issue (ICE), **Swing voters** tend to share *similar* opinions with **Loyal Democrats** across the remaining five issues. Interestingly, both these cohorts voted in favor of all five issues with a majority of voters choosing the *Strongly support* option.  
...
ggplot(WTHH.issue.long, aes(x=response, y=swing_voter)) + geom_point(alpha=0.4) +
  ...
  theme_bw()
```
The plots above showcase how the predicted probability of a registered voter being a swing voter varies with responses to each of the six issues. With the exception of GREENJOB and MARLEG, all the other fitted lines are fairly flat suggesting that we include just these two issues in our final model. However, just to be sure, let us do an explicit logistic regression fit including all six issue variables (without interaction) and look at the coefficients for each of these issues:
...
ggplot(WTHH.pop.long, aes(x=response, y=swing_voter)) + geom_point(alpha=0.4) +
  ...
  theme_bw()
```
The slope of the fitted lines decrease as we move from *POP_1* to *POP_2* ultimately flattening out as we reach *POP_3*. As before, just for our satisfaction, let us look at the slopes for each of these populism variables by explicitly fitting a logistic regression model for predicting the swing voters.
...
```
We see that the coefficients corresponding to *POP_2* and *POP_3* are very small. Hence, we can safely focus on just *POP_1* for our final model.

```{r}
WTHH.pop.logit <- glm(swing_voter ~ POP_1, weights=weight_DFP, family="quasibinomial", data=WTHH.pop)
display(WTHH.pop.logit)
```
We can interpret this model using the "divide by 4" 
...
WTHH.issue.logit.df$.pred <- 1 * (WTHH.issue.logit.df$.fitted > 0.165)
