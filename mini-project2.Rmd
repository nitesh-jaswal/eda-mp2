...
ggplot(dfp.q1, aes(x = RESPONSE, color = is_swing, fill = is_swing)) + stat_count((aes(weight = weight_DFP))) + facet_wrap(~ISSUE) + theme_bw() + guides(fill = guide_legend(title="Swing Voters"), color = FALSE) + scale_fill_manual(values = cb_palette) + labs(title="Some title") + ylab("COUNT")
```
Switch to D voters show a strong opinion for GREENJOB, GUNS, M4A and WELATH.
Switch to R voters show a strong opinion for ICE and M4A.
